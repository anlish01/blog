...
📂 整个博客 = 一个文件夹 = 核心 4 文件
   ...
   └─ 后台管理：admin.js + admin.css（响应式 SPA）
```

- ✅ **双击即开**：下载下来双击 `index.html` 就能写、能读，全程离线，连 `npm install` 都不需要
- ✅ **免费上云**：部署到 Cloudflare Workers + D1（免费额度），GitHub 推送即自动部署
- ✅ **功能完整**：写作后台 / 评论 / 加密 / 搜索 / 标签 / 归档 / RSS / 阅读统计 / 精选文章，一个不少
- ✅ **响应式管理后台**：PC 固定侧栏 / 移动端抽屉，六大页面（仪表盘 / 文章 / 评论 / 分类标签 / 媒体 / 设置）
...
- **双通道发布**：静态模式导出 `posts.js` + `feed.xml` + `sitemap.xml` 一键下载；云端模式一键「发布到云端」全站即时可见
- **导入 .md**：支持带 `---` frontmatter（title / date / tags / excerpt / password）的 Markdown 文件
- **文章列表管理**：直接切换**置顶 / 加密 / 删除**，无需进入编辑器
- **分类管理**：每篇文章可指定一个分类，支持重命名 / 删除（批量更新所有相关文章）
- **标签管理**：每篇文章可设置多个标签，支持重命名 / 删除

### 🔒 隐私与安全

- **文章加密**：PBKDF2 + AES-GCM 端到端加密，正文只存密文（列表接口也屏蔽），锁屏输入密码阅读
- **管理员安全**：密码只存 PBKDF2-SHA256 加盐哈希；会话令牌 7 天有效；同一 IP 连续失败 5 次锁定 15 分钟；首次部署自动初始化随机默认密码，登录后强制修改
- **静态模式哈希化**：本地密码 SHA-256 哈希存储（兼容旧版明文，登录后自动升级）
- **评论安全**：全文转义防 XSS、参数化查询防 SQL 注入、每 IP 频率限制、来源（Origin）校验、控制字符清洗、昵称/内容长度上限、管理员删除需令牌

### 💬 评论系统

- 云端 **D1 数据库全局评论**（所有访客共享），支持审核模式（新评论默认通过 / 待审核）
...
- **精选文章**：评论区下方自动推荐得分最高的文章（点赞×3 + 浏览×1 + 评论×5）
- **归档 / 标签云 / 上一篇下一篇 / 一键复制链接**：一个不少
- **上一篇 / 下一篇**：只有一条时自动隐藏空位，独占整行
...
- **仪表盘**：7 项数据统计卡片 + 30 天访问/评论趋势图
- **文章管理**：搜索 / 分类筛选 / 分页，一键置顶/取消置顶、加密/取消加密
- **编辑器**：Markdown 实时预览 + 分类/标签/封面/置顶/加密设置
- **评论管理**：全局评论列表，支持审核/删除
- **媒体资源库**：图片上传（base64 存 D1）、预览、复制链接
- **博客设置**：站点信息 / 个人资料 / 导航菜单（可视化编辑器 + JSON 双模式）
- **响应式布局**：PC 固定侧栏，移动端抽屉导航（992/640/420 三档断点）
...
双击 `public/index.html` → 右上角「✏️ 写文章」→ 设置一个管理密码（≥4 位）→ 开始写作。

> 静态模式没有后端，"发布" = 编辑器底部「导出全部」按钮一键下载 `posts.js` + `feed.xml` + `sitemap.xml`，放回 `public/` 目录覆盖旧文件即可。
...
4. 推送到 `main` → GitHub Actions 自动：建表 → 部署
5. 打开 `https://<你的域名>/admin` → 首次登录自动初始化随机默认密码 → 登录后强制修改密码
...
> 首次部署不再需要 `BLOG_ADMIN_SETUP_KEY`。系统自动初始化随机默认密码，登录后会提示修改。
...
| 后台管理 | 原生 JS 响应式 SPA（admin.js + admin.css） |
...
│   ├── style.css                # 前台样式（深色模式 + 响应式多断点）
│   ├── app.js                   # 前台逻辑（路由 / 写作 / 评论 / 加密 / 搜索…）
│   ├── admin.js                 # 管理后台 SPA（仪表盘 / 文章 / 评论 / 设置）
│   ├── admin.css                # 后台样式（响应式布局 + 暗色主题）
│   └── posts.js                 # 静态模式文章数据（Markdown）
├── functions/                   # Cloudflare API（Pages Functions / Workers 共用）
│   ├── api/                     # 路由：posts / comments / media / settings / admin / feed / sitemap
│   └── _lib/api-core.js         # API 核心（D1 存储 + 鉴权 + 安全）
├── worker.js                    # Cloudflare Workers 入口
├── migrations/                  # D1 表结构（CI 自动应用）
│   ├── 0001_init.sql            # 基础表（posts / comments / stats / admin_auth…）
│   ├── 0002_site_files.sql      # 站点文件存储
│   ├── 0003_cover_column.sql    # 封面图字段
│   ├── 0004_post_meta.sql       # 分类 / 发布状态字段
